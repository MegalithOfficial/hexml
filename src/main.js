          ...
          if (value.startsWith('<Date("') && value.endsWith('")>')) {
              ...
              switch (true) {
                  ...
                  case value.startsWith('<{{') && value.endsWith('}}>'):
                      ...
                      try {
                          const sanitizedValue = value.slice(3, -3).replace(/</g, '{').replace(/>/g, '}').trim();
                          const parsedValue = JSON.parse(`{${sanitizedValue}}`);
                          result[key] = parsedValue;
                      } catch (error) {
                          return logger.error(new EvaluationError(`Error parsing nested object for key "${key}": ${error.message}`));
                      }
                      break;
                  case value.startsWith('<{') && value.endsWith('}>'):
                      try {
                          const sanitizedValue = value.slice(2, -2).replace(/</g, '{').replace(/>/g, '}').trim();
                          const parsedValue = JSON.parse(`{${sanitizedValue}}`);
                          result[key] = parsedValue;
                      } catch (error) {
